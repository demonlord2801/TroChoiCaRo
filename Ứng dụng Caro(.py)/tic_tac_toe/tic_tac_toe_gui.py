...
class App(tk.Frame):
    def __init__(self, parent, dim, **kw):
        ...
        parent.title("Cờ ca rô")
        self.define_display_widgets()
        messagebox.showinfo("Info", "Chào mừng đến với trò chơi ")
        self.ttt_grid = Grid(self, {"tile_color": "red", "text": "Bảng Cờ"})

    def define_display_widgets(self):
        self.label = tk.Label(self, text="Cùng Chơi Cờ Ca Rô", font=('Verdana', 15, 'bold'))
        self.label.pack(side=tk.TOP)
        self.reset = tk.Button(self, text="Chơi lại", bg="grey", fg="white", command=self.reset_all)
...
class Grid(tk.Frame):
    ...
    def ai_move(self, start=None):
        ...
        self.score.config(text="Điểm của máy {}".format(s))
        index = 0
        for i in range(9):
    ...
    def check_if_game_ended(self, player):
        if is_draw(self.algo_value):
            messagebox.showinfo("Info", "Hòa !!")
            return True
        is_own, v = self.has_won()
        if is_own:
            messagebox.showinfo("Info", "{} đã thắng !".format(player))
            self.highlight(v)
            return True
        return False
    ...
    def set_algo(self):
        val = messagebox.askquestion("Info", "Bạn muốn máy bắt đầu trước chứ ?")
        self.algo_value = [-1] * 9
        self.turn = True
        if val == "yes":
